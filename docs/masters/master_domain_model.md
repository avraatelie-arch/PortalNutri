...
# 04. Modelo Clínico Implementado — Care Bounded Context

Esta seção documenta o modelo clínico **implementado até FEATURE-039**, alinhado a ADR-0016–0022 e ao código nos módulos `clinical/`, `patient/`, `nutrition/` e `appointment/`.

## Prontuário e ClinicalChart

| Conceito | Tipo | Status |
|----------|------|--------|
| **Prontuário** (termo de negócio) | Visão organizada do histórico clínico dentro de um Vínculo Clínico | Conceito de negócio preservado |
| **Prontuário** como Aggregate Root de escrita | Modelo superseded (v1.0 de `master_aggregates.md`) | **Não implementado** |
| **ClinicalChart** | Read model / compositor query-side (ADR-0019) | Planejado — FEATURE-040 |

O termo **Prontuário** permanece na linguagem de negócio para descrever a visão clínica do paciente. Na implementação, essa visão será composta por **ClinicalChart** a partir de múltiplos aggregates de escrita — nunca por um único aggregate monolítico.

## Decomposição física vs Bounded Context

O Care BC possui **um único proprietário conceitual** dos conceitos clínicos, implementado em módulos físicos:

- `clinical/` — núcleo de escrita clínica (10 Aggregate Roots)
- `patient/` — cadastro e vínculo clínico
- `nutrition/` — perfil profissional
- `appointment/` — agendamento operacional

Módulos físicos **não são Bounded Contexts independentes**. Anti-corruption é garantida por **directory ports** na camada de aplicação.

## Aggregates de escrita — cluster session-bound

| Código | Termo PT | Papel |
|--------|----------|-------|
| `ClinicalEncounter` | Encontro Clínico | Sessão clínica em curso ou finalizada |
| `Anamnesis` | Anamnese | Coleta estruturada por seções |
| `AnthropometricAssessment` | Avaliação Antropométrica | Medidas antropométricas imutáveis |
| `BodyCompositionAssessment` | Avaliação de Composição Corporal | Composição corporal imutável |
| `ClinicalEvolution` | Evolução Clínica | **Descreve e narra** mudança clínica longitudinal (ADR-0021) |

## Aggregates de escrita — cluster patient-scoped

| Código | Termo PT | Papel |
|--------|----------|-------|
| `NutritionDiagnosis` | Diagnóstico Nutricional | Diagnóstico nutricional estruturado |
| `ClinicalObjective` | Objetivo Clínico | Meta terapêutica com lifecycle próprio |
| `MealPlan` | Plano Alimentar | Estratégia alimentar (contém `MealPlanMeal`) |
| `Prescription` | Prescrição Nutricional | Instruções emitidas (contém `PrescriptionLine`) |
| `OutcomeTracking` | Acompanhamento de Resultado | **Avalia** progresso em direção a um objetivo (ADR-0022) |

## Separações semânticas críticas

### Appointment vs ClinicalEncounter

- **Appointment** (`appointment/`) — agendamento administrativo: horário, confirmação, reagendamento, no-show.
- **ClinicalEncounter** (`clinical/`) — sessão clínica propriamente dita: atendimento, anamnese, evolução vinculada.

### ClinicalEvolution vs OutcomeTracking

- **ClinicalEvolution** narra o delta clínico intermomentos (texto estruturado, observações, conduta).
- **OutcomeTracking** emite um **julgamento clínico estruturado** (`OutcomeAssessment`) sobre progresso de um `ClinicalObjective` específico.

### GOAL_ACHIEVED vs conclusão de ClinicalObjective

- `GOAL_ACHIEVED` é um valor de **`OutcomeAssessment`** dentro de `OutcomeTracking`.
- **Não completa automaticamente** o `ClinicalObjective` (lifecycle `ACTIVE → COMPLETED` permanece independente).
- **Não dispara writes cross-aggregate** — regra validada em testes de domínio e aplicação (FEATURE-039).

## Conceitos Care ainda não implementados

Protocolo Aplicado, Solicitação/Resultado de Exame, Indicador Clínico standalone, ClinicalChart (FEATURE-040).

---

# 05. Glossário Oficial do Domínio
...
Na linguagem de negócio, o Prontuário preserva histórico, objetivos, consultas, avaliações, evoluções, protocolos, prescrições, exames e indicadores clínicos do paciente naquele contexto.

**Na implementação atual (FEATURE-039):** o Prontuário **não é um Aggregate Root de escrita**. A visão unificada será composta query-side por **ClinicalChart** (ADR-0019; FEATURE-040). Os registros clínicos são persistidos por múltiplos Aggregate Roots independentes documentados em `master_aggregates.md` §04.

---

## ClinicalChart

Compositor query-side / read model que agregará a timeline clínica do paciente a partir de queries sobre os aggregates de escrita existentes.

Conforme ADR-0019: **não possui commands, lifecycle próprio ou persistência independente**. Planejado em FEATURE-040 (BACKLOG-007). **Não confundir com Aggregate Root.**

---

## ClinicalEncounter

Aggregate Root session-bound que representa o **Encontro Clínico** — a sessão de atendimento em curso ou finalizada.

Distinto de **Appointment** (agendamento operacional). Lifecycle: `OPEN → FINISHED | CANCELLED`.

---

## Anamnesis

Aggregate Root session-bound para coleta estruturada de informações clínicas por seções. Lifecycle: `DRAFT → COMPLETED`.

Termo de negócio: **Anamnese**.

---

## AnthropometricAssessment

Aggregate Root session-bound para registro imutável de medidas antropométricas. Termo de negócio: **Avaliação Antropométrica**.

---

## BodyCompositionAssessment

Aggregate Root session-bound para registro imutável de composição corporal. Termo de negócio: **Avaliação de Composição Corporal**.

---

## NutritionDiagnosis

Aggregate Root patient-scoped para diagnóstico nutricional estruturado. Lifecycle: `DRAFT → CONFIRMED | CANCELLED`. Termo de negócio: **Diagnóstico Nutricional**.

---

## ClinicalEvolution

Aggregate Root session-bound que **descreve e narra** mudança clínica longitudinal (Model B — ADR-0021). Lifecycle: `DRAFT → FINALIZED | CANCELLED`. Conteúdo `FINALIZED` imutável.

Termo de negócio: **Evolução Clínica**. Distinto de OutcomeTracking.

---

## OutcomeTracking

Aggregate Root patient-scoped que **avalia** progresso em direção a um `ClinicalObjective` específico, emitindo julgamento clínico estruturado (`OutcomeAssessment`). Lifecycle: `DRAFT → RECORDED | CANCELLED`. Conteúdo `RECORDED` imutável (ADR-0022).

Termo de negócio: **Acompanhamento de Resultado**. Distinto de ClinicalEvolution. Write model **não contém referências a evidências** — composição query-side deferida (BACKLOG-017).

---

## OutcomeAssessment

Value Object / enum que representa o julgamento clínico estruturado em `OutcomeTracking`. Valores incluem: `ON_TRACK`, `PARTIAL`, `STABLE`, `STALLED`, `REGRESSED`, `GOAL_ACHIEVED`, `NOT_EVALUABLE`.

`GOAL_ACHIEVED` indica que o objetivo foi alcançado na avaliação momentânea — **não completa automaticamente** o `ClinicalObjective`.

---

## Appointment

Aggregate Root no módulo `appointment/` para **agendamento operacional** (horário, confirmação, reagendamento). Distinto de `ClinicalEncounter` (sessão clínica).

---

## Objetivo Clínico

Finalidade terapêutica definida para um paciente, implementada como Aggregate Root **ClinicalObjective** (cluster patient-scoped).

Organiza a jornada clínica agrupando consultas, avaliações, evoluções, planos, prescrições e acompanhamentos de resultado relacionados a um determinado objetivo de cuidado. Lifecycle: `DRAFT → ACTIVE ⇄ PAUSED → COMPLETED | CANCELLED`.

Conclusão (`COMPLETED`) é decisão independente do julgamento `GOAL_ACHIEVED` em OutcomeTracking.
...
Estratégia nutricional prática implementada como Aggregate Root **MealPlan** (cluster patient-scoped). Contém entidades subordinadas **MealPlanMeal**.

Define refeições, alimentos, substituições, horários, recomendações e orientações alimentares. Lifecycle: `DRAFT → ACTIVE | CANCELLED`.

---

## Prescrição Nutricional

Conduta nutricional formal implementada como Aggregate Root **Prescription** (cluster patient-scoped). Contém entidades subordinadas **PrescriptionLine**.

Poderá conter suplementos, manipulados, fitoterápicos, dosagens, frequência, duração e observações profissionais. Lifecycle: `DRAFT → ISSUED | CANCELLED`; verbo de publicação: `emit()`.
