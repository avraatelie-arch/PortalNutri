...
import { UpdateNutritionistProfileHandler } from './update-nutritionist-profile.handler.js';
...
describe('UpdateNutritionistProfileHandler', () => {
  it('updates specialty and bio', async () => {
    const { nutritionistRepository, created } = await seedActiveNutritionist();
    const handler = new UpdateNutritionistProfileHandler(
      nutritionistRepository,
      noopEventDispatcher,
    );

    const result = await handler.execute(
      new UpdateNutritionistProfileCommand({
        nutritionistId: created.id,
        specialty: 'Sports Nutrition',
        bio: 'Updated bio.',
      }),
    );

    assert.equal(result.specialty, 'Sports Nutrition');
    assert.equal(result.bio, 'Updated bio.');
  });

  it('dispatches NutritionistProfileUpdated only when profile changes', async () => {
    const { nutritionistRepository, created } = await seedActiveNutritionist();
  ...
  it('clears bio when set to null', async () => {
    const { nutritionistRepository, created } = await seedActiveNutritionist();
    const handler = new UpdateNutritionistProfileHandler(
      nutritionistRepository,
      noopEventDispatcher,
  ...
  it('throws NutritionistValidationError when nutritionist is inactive', async () => {
    const { nutritionistRepository, created } = await seedActiveNutritionist();
