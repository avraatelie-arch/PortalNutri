...
import { ActivateClinicalObjectiveCommand } from './activate-clinical-objective.command.js';
import { ActivateClinicalObjectiveHandler } from './activate-clinical-objective.handler.js';
...
describe('ActivateClinicalObjectiveHandler', () => {
  it('activates a draft objective with title', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    await seedDraftObjective(repository);
    const handler = new ActivateClinicalObjectiveHandler(
      repository,
    ...
    assert.equal(result.status, ClinicalObjectiveStatusValue.Active);
    assert.equal(result.activatedAt, LATER.toISOString());
    assert.equal(result.version, 2);
  });

  it('throws when objective is not draft', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    ...
    await assert.rejects(
      ...
      ClinicalObjectiveNotDraftError,
    );
  });

  it('throws ClinicalObjectiveNotFoundError for wrong tenant', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    await seedDraftObjective(repository);
    const handler = new ActivateClinicalObjectiveHandler(
      repository,
