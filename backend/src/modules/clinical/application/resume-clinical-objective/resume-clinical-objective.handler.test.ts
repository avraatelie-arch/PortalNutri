...
import { ResumeClinicalObjectiveHandler } from './resume-clinical-objective.handler.js';
...
describe('ResumeClinicalObjectiveHandler', () => {
  it('resumes a paused objective', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    await seedPausedObjective(repository);
    const handler = new ResumeClinicalObjectiveHandler(
      repository,
    ...
    const result = await handler.execute(
      new ResumeClinicalObjectiveCommand({
        tenantId: TENANT_ID,
  ...
  it('throws when objective is not paused', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    ...
    await assert.rejects(
      ...
      ClinicalObjectiveNotPausedError,
    );
  });
});
