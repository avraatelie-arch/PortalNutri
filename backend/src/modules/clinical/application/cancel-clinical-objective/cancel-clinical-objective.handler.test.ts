...
import { CancelClinicalObjectiveHandler } from './cancel-clinical-objective.handler.js';
...
describe('CancelClinicalObjectiveHandler', () => {
  it('cancels a draft objective', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    await seedDraftObjective(repository);
    const handler = new CancelClinicalObjectiveHandler(
      repository,
    ...
    const result = await handler.execute(
      new CancelClinicalObjectiveCommand({
        tenantId: TENANT_ID,
        clinicalObjectiveId: OBJECTIVE_ID,
      }),
    );

    assert.equal(result.status, ClinicalObjectiveStatusValue.Cancelled);
