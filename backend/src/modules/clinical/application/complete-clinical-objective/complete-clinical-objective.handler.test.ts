...
import { CompleteClinicalObjectiveHandler } from './complete-clinical-objective.handler.js';
...
describe('CompleteClinicalObjectiveHandler', () => {
  it('completes an active objective', async () => {
    const repository = new InMemoryClinicalObjectiveRepository();
    await seedActiveObjective(repository);
    const handler = new CompleteClinicalObjectiveHandler(
      repository,
    ...
    const result = await handler.execute(
      new CompleteClinicalObjectiveCommand({
        tenantId: TENANT_ID,
        clinicalObjectiveId: OBJECTIVE_ID,
      }),
    );

    assert.equal(result.status, ClinicalObjectiveStatusValue.Completed);
