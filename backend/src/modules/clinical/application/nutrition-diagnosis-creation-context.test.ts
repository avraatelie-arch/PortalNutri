...
import { ClinicalEncounterNotOpenForNutritionDiagnosisError } from './errors/clinical-encounter-not-open-for-clinical-objective.error.js';
import { NutritionDiagnosisOriginAnamnesisEncounterMismatchError } from './errors/clinical-objective-origin-anamnesis-encounter-mismatch.error.js';
import { PatientInactiveForNutritionDiagnosisError } from './errors/patient-inactive-for-clinical-objective.error.js';
import { TenantNotFoundForNutritionDiagnosisError } from './errors/tenant-not-found-for-clinical-objective.error.js';
...
const errors = createNutritionDiagnosisCreationContextErrors();

describe('buildNutritionDiagnosisCreationContext', () => {
  it('returns a frozen context when references are valid', async () => {
    const directories = seedDirectories();

    const context = await buildNutritionDiagnosisCreationContext({
      ...directories,
      request: {
        tenantId: TENANT_ID,
        patientId: PATIENT_ID,
  ...
  it('throws when patient is inactive', async () => {
    ...
    await assert.rejects(
      ...
      PatientInactiveForNutritionDiagnosisError,
    );
  });
  ...
  it('throws when origin encounter is not open', async () => {
    ...
    await assert.rejects(
      ...
      ClinicalEncounterNotOpenForNutritionDiagnosisError,
    );
  });
  ...
  it('throws when origin anamnesis does not match encounter', async () => {
    ...
    await assert.rejects(
      ...
      NutritionDiagnosisOriginAnamnesisEncounterMismatchError,
    );
  });
});
