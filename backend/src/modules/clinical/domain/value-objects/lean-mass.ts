...
export class LeanMass {
  ...
  static create(raw: string): LeanMass {
    const value = parseClinicalDecimal(raw, 'leanMassKg', MASS_SCALE);
    validateNonNegativeMass(value, 'leanMassKg');
    return new LeanMass(value);
  }

  static createOptional(raw: string | null | undefined): LeanMass | null {
    const value = parseOptionalClinicalDecimal(raw, 'leanMassKg', MASS_SCALE);
  ...
  static fromDecimal(value: Decimal): LeanMass {
    return new LeanMass(value);
  }
