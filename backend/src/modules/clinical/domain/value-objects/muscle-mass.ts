...
export class MuscleMass {
  ...
  static create(raw: string): MuscleMass {
    const value = parseClinicalDecimal(raw, 'muscleMassKg', MASS_SCALE);
    validateNonNegativeMass(value, 'muscleMassKg');
    return new MuscleMass(value);
  }

  static createOptional(raw: string | null | undefined): MuscleMass | null {
    const value = parseOptionalClinicalDecimal(raw, 'muscleMassKg', MASS_SCALE);
  ...
  static fromDecimal(value: Decimal): MuscleMass {
    return new MuscleMass(value);
  }
