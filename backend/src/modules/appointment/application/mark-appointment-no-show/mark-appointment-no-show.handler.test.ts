...
import { MarkAppointmentNoShowHandler } from './mark-appointment-no-show.handler.js';
...
describe('MarkAppointmentNoShowHandler', () => {
  it('marks a scheduled appointment as no-show', async () => {
    ...
    assert.equal(result.status, AppointmentStatus.NoShow);
  });

  it('mark no-show is idempotent', async () => {
    const { appointmentRepository, scheduled } = await seedScheduledAppointment();
  ...
  it('rejects marking no-show on completed appointment', async () => {
    const { appointmentRepository, scheduled } = await seedScheduledAppointment();
    const handler = new MarkAppointmentNoShowHandler(
      appointmentRepository,
      new FixedClock(NOW),
