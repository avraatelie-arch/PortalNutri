...
import { CancelAppointmentCommand } from '../cancel-appointment/cancel-appointment.command.js';
import { CancelAppointmentHandler } from '../cancel-appointment/cancel-appointment.handler.js';
...
import { CompleteAppointmentHandler } from './complete-appointment.handler.js';
...
describe('CompleteAppointmentHandler', () => {
  it('completes a scheduled appointment', async () => {
    ...
    const handler = new CompleteAppointmentHandler(
      appointmentRepository,
      new FixedClock(NOW),
      noopEventDispatcher,
    );
    ...
    assert.equal(completed.status, AppointmentStatus.Completed);
  ...
  it('rejects completing a cancelled appointment', async () => {
    ...
    await new CancelAppointmentHandler(
      appointmentRepository,
      new FixedClock(NOW),
      noopEventDispatcher,
    ).execute(
