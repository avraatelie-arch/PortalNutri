...
import { CancelAppointmentHandler } from './cancel-appointment.handler.js';
...
describe('CancelAppointmentHandler', () => {
  it('cancels an appointment with reason', async () => {
    ...
    const handler = new CancelAppointmentHandler(
      appointmentRepository,
      new FixedClock(NOW),
      noopEventDispatcher,
    );
    ...
    assert.equal(cancelled.cancellationReason, 'Patient unavailable');
  ...
  it('rejects empty cancellation reason', async () => {
    const { appointmentRepository, scheduled } = await seedScheduledAppointment();
    const handler = new CancelAppointmentHandler(
      appointmentRepository,
      new FixedClock(NOW),
